...
(defn write [conn msg]
  (let [msg (str msg "\r\n")]
    (if (< message-max-character-length (count msg))
      (throw (Exception.
              (str "Message, " msg ", exceeds "
                   message-max-character-length
                   " character IRC message protocol limit."))))
    (doto (:out @conn)
      (.print msg)
      (.flush)))
  conn)

(defn build-irc-message
  ...
  (let [parsed-or-failure (ircparse.core/message message)]
    (if (:failure parsed-or-failure)
      ...
      (let [parsed parsed-or-failure]
        (cond
         (= "PING" (:command parsed))
         {:reply [(build-irc-message
                   {:command "PONG" :trailing (:trailing-params parsed)})]}
         (= "PRIVMSG" (:command parsed))
         {:message [{:from (:client-nickname parsed)
                     :channel  (apply str (:middle-params parsed))
                     :body (:trailing-params parsed)}]}
         :else
  ...
  (while (nil? (:exit @conn))
    (let [msg (.readLine (:in @conn))]
      (if (nil? msg)
        ...
        (let [msg (str msg "\r\n")
              command-map (handle-message msg)]
...
(defn send-privmsg [conn body]
  (doall (map #(write conn (make-privmsg (clojure.string/join %)))
              (remove empty?
                      (partition-all
                       (- (- message-max-character-length 2) 15) body)))))
